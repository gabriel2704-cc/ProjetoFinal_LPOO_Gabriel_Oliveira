...
class ProdutoDAO:
    ...
    def salvar(self, produto: Produto) -> bool:
        ...
        sql = """INSERT INTO produtos (nome, preco, quantidade_atual, quantidade_minima, categoria, fornecedor_codigo) 
                 VALUES (%s, %s, %s, %s, %s, %s) RETURNING codigo;"""
        try:
            cursor = conexao.cursor()
            cursor.execute(sql, (produto.nome, produto.preco, produto.quantidade_atual, 
                                 produto.quantidade_minima, produto.categoria, produto.fornecedor.codigo))
    ...
    def atualizar(self, produto: Produto) -> bool:
        ...
        sql = """UPDATE produtos SET nome=%s, preco=%s, quantidade_atual=%s, quantidade_minima=%s, 
                 categoria=%s, fornecedor_codigo=%s WHERE codigo=%s;"""
        try:
            cursor = conexao.cursor()
            cursor.execute(sql, (produto.nome, produto.preco, produto.quantidade_atual, 
                                 produto.quantidade_minima, produto.categoria, produto.fornecedor.codigo, produto.codigo))
            conexao.commit()
            cursor.close()
            return True
        except Exception as e:
    ...
    def listar(self) -> list:
        ...
        sql = "SELECT codigo, nome, preco, quantidade_atual, quantidade_minima, categoria, fornecedor_codigo FROM produtos;"
        try:
            cursor = conexao.cursor()
            cursor.execute(sql)
            ...
            for linha in resultados:
                ...
                fornecedor_obj = self.fornecedor_dao.buscar_por_id(linha[6])
                
                p = Produto(codigo=linha[0], nome=linha[1], preco=linha[2], quantidade_atual=linha[3], 
                            quantidade_minima=linha[4], categoria=linha[5], fornecedor=fornecedor_obj)
    ...
    def buscar_por_id(self, codigo: int) -> Produto:
        ...
        sql = "SELECT codigo, nome, preco, quantidade_atual, quantidade_minima, categoria, fornecedor_codigo FROM produtos WHERE codigo = %s;"
        try:
            cursor = conexao.cursor()
            cursor.execute(sql, (codigo,))
